...
3. **Post the synthesized review** as a comment on the PR. Build the body as a string, write it to a temp file via the Write tool (path `.claude/scratch/review-pr-$ARGUMENTS-team.md`), then post with `--body-file`:

```bash
gh pr comment $ARGUMENTS --body-file .claude/scratch/review-pr-$ARGUMENTS-team.md
```
