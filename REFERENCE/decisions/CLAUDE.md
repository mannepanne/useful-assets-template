...
- [2026-05-24 — Sentinel in committed profile + gitignored `.local` sibling for first-run personalisation](./2026-05-24-personal-profile-setup-sentinel.md) — why `personal-profile.md` always ships with a `<!-- profile_status: default -->` sentinel instead of being modified on setup, why the operator's actual profile lives in a gitignored `personal-profile.local.md`, and why this mirrors the `project-config.local.json` pattern. Operations reference at [`REFERENCE/profile-setup-hook.md`](../profile-setup-hook.md).
