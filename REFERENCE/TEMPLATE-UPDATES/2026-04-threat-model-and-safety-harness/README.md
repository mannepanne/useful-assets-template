...
- **Dispatcher Read-then-Write fallback:** `/review-pr` and `/review-pr-team` no longer use `rm -f .claude/scratch/...` to clear stale temp files (which prompted because `rm` isn't allowlisted). They Read first, then Write — silent under default permissions.
- **Project-relative scratch directory:** review skills write their intermediate comment-body files to `.claude/scratch/` rather than `/tmp/`. The directory is gitignored (`*\n!.gitignore` inside `.claude/scratch/.gitignore`), and the allow-list rules become `Write(.claude/scratch/*)` + `Read(.claude/scratch/*)` — broader than the old per-skill patterns, which means `/review-spec` (and any future skill) can use the same scratch space without separate rules. Sidesteps an intermittent matcher quirk where absolute `/tmp/...` paths were sometimes displayed in `../../../../../../tmp/...` traversal form and failed allow-list matching (see `SPECIFICATIONS/INVESTIGATION-claude-code-write-path-normalisation.md`). Root cause unresolved; symptom no longer surfaces because the path is now project-relative.
...
- **Allowlist tuning** in `.claude/settings.json` for git-pipe forms, `git fetch`, `Write(.claude/scratch/*)`, `Read(.claude/scratch/*)`, etc.
...
- `.claude/scratch/.gitignore` — single file, two lines (`*\n!.gitignore`). Keeps the directory tracked but ignores all contents. Review skills write their intermediate comment-body files here; the gitignored directory means artifacts don't leak into commits. Without this file, the directory wouldn't exist in fresh clones and the first Write would have to create the parent.
...
- **`permissions.allow` additions** — multiple new entries across two thematic groups, plus a `_comment` field describing the threat-model assumption:
  - **Review-tooling entries** (the silent-reviews work): `Bash(git fetch *)`, `Bash(git -C * log/show/diff/status *)`, `Bash(git show * | sed -n *)` and the related git-pipe siblings, `Bash(gh pr diff * | grep *)`, `Write(.claude/scratch/*)`, `Read(.claude/scratch/*)`. The scratch rules cover any review skill writing intermediate files into the project-relative `.claude/scratch/` directory (gitignored). Older versions of this packet pinned to `/tmp/review-pr-*` and `/tmp/spec-review-*` — derivative projects upgrading from the older form should swap those two rules for the two `.claude/scratch/*` rules and add `.claude/scratch/.gitignore`.
...
- PR #22 added a Read-then-Write fallback paragraph to both files, removing the need for `rm -f /tmp/...` cleanup (later updated to `rm -f .claude/scratch/...` when the scratch directory moved project-relative). The paragraph appears under the "Posting the comment" section in `review-pr/SKILL.md` and the equivalent in `review-pr-team/SKILL.md`. The skill text now references `.claude/scratch/review-pr-...` paths (not `/tmp/review-pr-...`) — derivative projects upgrading from an earlier shape of this packet should sweep the skill files for any remaining `/tmp/` paths and replace with `.claude/scratch/`. If the local skill files don't have this paragraph, add it. If they have a different stale-file workaround (e.g. `rm -f`), replace it with the Read-then-Write text.
...
test -f .claude/scratch/.gitignore                              # scratch dir present
grep -q '\.claude/scratch/' .claude/settings.json               # scratch allow-list applied
grep -q '\.claude/scratch/' .claude/skills/review-pr/SKILL.md   # review skills point at scratch dir
grep -q '\.claude/scratch/' .claude/skills/review-pr-team/SKILL.md
