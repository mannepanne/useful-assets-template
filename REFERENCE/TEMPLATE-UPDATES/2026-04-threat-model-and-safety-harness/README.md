...
- **Allowlist tuning** in `.claude/settings.json` for git-pipe forms, `git fetch`, `Write(/tmp/review-pr-*)`, `Write(/tmp/spec-review-*)`, etc.
...
- **`permissions.allow` additions** — multiple new entries across two thematic groups, plus a `_comment` field describing the threat-model assumption:
  ...
  - **Validation tooling entries** (silent JSON syntax checks Claude reaches for after editing settings/config files): `Bash(python3 -m json.tool:*)`, `Bash(python3 -m json.tool * > /dev/null)`, `Bash(python3 -m json.tool * > /dev/null && echo *)`, `Bash(python3 -m json.tool * && echo *)`. Pinned narrowly to `python3 -m json.tool` — a stdlib module that reads JSON from a file and pretty-prints it (read-only, no network, no shell). **Do not** broaden to `Bash(python3:*)` or `Bash(python3 -m *)`: that would silently allow `python3 -c "import os; os.system(...)"` (full arbitrary code execution) and any other Python module. The same risk delineation applies if you add `jq` later — pin to the specific subcommand shapes you trust, not to the binary.
