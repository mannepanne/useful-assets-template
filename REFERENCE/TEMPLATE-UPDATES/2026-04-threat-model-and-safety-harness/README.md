...
- **Project-relative scratch directory:** review skills write their intermediate comment-body files to a top-level `SCRATCH/` directory rather than `/tmp/`. The directory contents are gitignored (`*\n!.gitignore` inside `SCRATCH/.gitignore`), and the allow-list rules become `Write(/SCRATCH/*)` + `Read(/SCRATCH/*)` — broader than the old per-skill patterns, which means `/review-spec` (and any future skill) can use the same scratch space without separate rules. The leading-slash form is load-bearing: in Claude Code permission globs, a leading `/` means "project-root-relative" (not filesystem-absolute — that's `//`), and the Write/Read tools always pass an absolute path that the matcher compares against. The no-slash form `Write(SCRATCH/*)` silently fails to match and surfaces a manual approval prompt on every scratch write — derivative projects upgrading from an earlier draft of this packet that used the no-slash form should swap both entries to the leading-slash variant. Sidesteps two distinct problems: (1) an intermittent matcher quirk where absolute `/tmp/...` paths were sometimes displayed in `../../../../../../tmp/...` traversal form and failed allow-list matching (see `SPECIFICATIONS/INVESTIGATION-claude-code-write-path-normalisation.md`); and (2) a settings-self-modification approval gate that fires on any Write under `.claude/`, regardless of allowlist entries — which is why the directory lives at the repo root rather than inside `.claude/`. Root cause of (1) unresolved; symptom no longer surfaces because the path is now project-relative.
...
- **`permissions.allow` additions** — multiple new entries across two thematic groups, plus a `_comment` field describing the threat-model assumption:
  - **Review-tooling entries** (the silent-reviews work): `Bash(git fetch *)`, `Bash(git -C * log/show/diff/status *)`, `Bash(git show * | sed -n *)` and the related git-pipe siblings, `Bash(gh pr diff * | grep *)`, `Write(/SCRATCH/*)`, `Read(/SCRATCH/*)`. The scratch rules cover any review skill writing intermediate files into the top-level `SCRATCH/` directory (gitignored contents). The leading slash on `/SCRATCH/*` is load-bearing — it means "project-root-relative" in the Claude Code permission grammar, and the no-slash form `Write(SCRATCH/*)` silently fails to match because Write/Read tools always pass an absolute path. Older versions of this packet pinned to `/tmp/review-pr-*` and `/tmp/spec-review-*` — derivative projects upgrading from the older form should swap those two rules for the two `/SCRATCH/*` rules and add `SCRATCH/.gitignore`. Projects upgrading from an even-earlier draft of THIS packet that shipped the no-slash form `Write(SCRATCH/*)` / `Read(SCRATCH/*)` should rewrite both entries to the leading-slash form. An even-earlier intermediate shape used `.claude/scratch/*`; that form triggered Claude Code's settings-self-modification approval prompt on every Write and was abandoned for the repo-root location.
