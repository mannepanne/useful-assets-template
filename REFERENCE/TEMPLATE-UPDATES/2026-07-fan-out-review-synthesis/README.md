...
- `.claude/agents/security-specialist.md`, `product-reviewer.md`, `architect-reviewer.md`, `technical-writer.md` — Role line no longer says "as part of an agent team"; `## Team Collaboration` → `## Reporting to the orchestrator`. In `security-specialist` and `product-reviewer`, the final "Be collaborative" review standard became "Be balanced".
- `.claude/agents/requirements-auditor.md`, `technical-skeptic.md`, `devils-advocate.md` — same `## Team Collaboration` → `## Reporting to the orchestrator` swap, with the cross-reviewer instructions rewritten as self-directed guidance.
